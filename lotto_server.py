    ...
    with sync_playwright() as p:
        ...
        browser = p.chromium.launch(
            ...
            args=[
                ...
                "--disable-dev-shm-usage", # 서버 메모리 최적화 옵션 (중요)
                "--disable-gpu"
            ]
        )
        # 뷰포트는 크롬 모바일 뷰 혹은 PC 모바일 해상도
...
def open_browser(): 
    ...
    print("\n" + "🚀"*32)
    print("      인터넷 터널링(ngrok) 서버 생성 중...")
    
    try:
        # ngrok으로 퍼블릭 URL 생성
        public_url = ngrok.connect(5000).public_url
        print("\n" + "="*65)
        print("🌐 [글로벌 환경 접속] 외부(LTE/5G 등)에서 아래 주소로 접속하세요:")
        print(f"👉 {public_url} 👈")
        print("="*65)
        
        # 종료 시 ngrok 연결 닫기 (메모리 릭 방지)
        atexit.register(ngrok.disconnect, public_url)
    except Exception as e:
        print("\n❌ 인터넷 서버(ngrok) 생성 실패: ", e)
        print("💡 ngrok이 설치되어있지 않거나 권한 문제가 있을 수 있습니다.")
    
    print("\n" + "="*65)
    print("🏠 [같은 와이파이 접속] 화면과 동일 네트워크(공유기) 접속용:")
